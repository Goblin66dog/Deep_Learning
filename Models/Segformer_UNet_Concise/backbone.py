...
class mit_b0(MixVisionTransformer):
    def __init__(self, pretrained = False, in_channels = 5):
        super(mit_b0, self).__init__(
            embed_dims=[32, 64, 160, 256], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 2, 2], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.1, in_chans = in_channels)
        if pretrained:
...
class mit_b3(MixVisionTransformer):
    def __init__(self, pretrained = False, in_channels = 5):
        ...
        if pretrained:
            print("Load backbone weights")
            # self.load_state_dict(torch.load("D:\Project\Deep_Learning\model\Segformer_UNet\segformer_b3_backbone_weights.pth"), strict=False)
            print('Loading weights into state dict...')
            device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
            model_dict = self.state_dict()
            pretrained_dict = torch.load(
                "D:\Github_Repo\Deep_Learning\Models\Segformer\segformer_b3_backbone_weights.pth",
                map_location=device)
            a = {}
